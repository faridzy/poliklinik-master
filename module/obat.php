...
//            echo "Connection success";
    ...
    public function create($page, $params = []) {
        try {
            ...
            $sql    = "INSERT INTO $this->table (ID, ID_POLI, NAME, PRICE, DESCRIPTION, STOCK, UNIT) VALUES 
                      ($id, $params[id_poli], '$params[name]', $params[price], '$params[description]', 
                      $params[stock], '$params[unit]')";
            $query  = $this->conn->prepare($sql);
            $query->execute();
        ...
        try {
            try {
                $sql = "UPDATE $this->table set ID_POLI = $params[id_poli], NAME = '$params[name]', 
                    PRICE = '$params[price]', DESCRIPTION = '$params[description]', STOCK = '$params[stock]',
                    UNIT = '$params[unit]' 
                    WHERE ID = $id";
                $query = $this->conn->prepare($sql);
                $query->execute();
